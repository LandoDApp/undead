...
export async function meetupRoutes(app) {
    ...
    app.get('/', async () => {
        const result = await db
            ...
            .where(and(eq(meetups.isActive, true), gte(meetups.scheduledAt, new Date())));
    ...
    app.delete('/:id/checkin', async (request) => {
        ...
        await db
            ...
            .where(and(eq(meetupCheckins.meetupId, id), eq(meetupCheckins.userId, request.user.id)));
        return { success: true };
    });
