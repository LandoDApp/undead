...
export async function pointRoutes(app) {
    app.addHook('preHandler', app.authenticate);
    // Get collectible points nearby
    app.get('/nearby', async (request) => {
        const query = nearbySchema.parse(request.query);
        const points = await pointsService.getOrCreatePointsInArea({ latitude: query.lat, longitude: query.lon }, POINT_NEARBY_RADIUS);
        return { success: true, data: { points } };
    });
    // Collect a point
    app.post('/collect', async (request) => {
        const body = collectSchema.parse(request.body);
        const result = await pointsService.collectPoint(request.user.id, body.pointId, body.playerLat, body.playerLon);
