...
export async function ghoulRoutes(app: FastifyInstance) {
  ...
  app.post('/route', async (request) => {
    ...
    const result = await getRoute(
      { latitude: body.ghoulLat, longitude: body.ghoulLon },
  ...
  app.post('/catch', async (request) => {
    ...
    const result = await processGhoulCatch(
      request.user.id,
      body.ghoulLat,
      body.ghoulLon,
      body.playerLat,
