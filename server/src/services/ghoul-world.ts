...
import {
  GHOULS_PER_CELL,
  GHOUL_RESPAWN_TIME,
  GHOUL_SPAWN_DISTANCE_MIN,
  GHOUL_SPAWN_DISTANCE_MAX,
} from '@undead/shared';
...
async function reviveExpiredGhouls(cells: string[]): Promise<void> {
  ...
  await db
    ...
    .where(
      and(
        inArray(ghouls.gridCell, cells),
        eq(ghouls.isAlive, false),
        lte(ghouls.deadUntil, Date.now())
      )
    );
}
...
async function fillCells(cells: string[]): Promise<void> {
  ...
  const spawnPromises: Promise<(typeof ghouls.$inferInsert)>[] = [];
...
export async function getOrCreateGhoulsInArea(
  center: Coordinate,
  radiusM: number
): Promise<ServerGhoul[]> {
...
export async function killGhoul(ghoulId: string): Promise<boolean> {
