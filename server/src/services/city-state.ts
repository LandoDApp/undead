...
import { spendHerbs, spendCrystals, getPlayerResourceBalance } from './resources.js';
...
export async function getCityStateById(id: string) {
  const results = await db.select().from(cityStates).where(eq(cityStates.id, id));
  return results[0] ?? null;
}

export async function enterCityState(userId: string, zoneId: string) {
  const zone = await getCityStateById(zoneId);
  if (!zone) throw new Error('City-state not found');
  ...
  // Quest progress
  import('./quests.js').then((m) => m.incrementQuestProgress(userId, 'visit', 1)).catch(() => {});

  return { ...zone, charge: newCharge };
}

export async function exitCityState(userId: string, zoneId: string) {
  // Close open visit
...
export async function reconquerCityState(userId: string, zoneId: string) {
  const zone = await getCityStateById(zoneId);
  if (!zone) throw new Error('City-state not found');
  if (!zone.isFallen) throw new Error('City-state is not fallen');
...
export async function healCityState(userId: string, zoneId: string, amount: number) {
  const zone = await getCityStateById(zoneId);
  if (!zone) throw new Error('City-state not found');
  if (zone.isFallen) throw new Error('Cannot heal a fallen city-state');

  const hpToHeal = Math.min(amount, zone.maxCharge - zone.charge);
  if (hpToHeal <= 0) throw new Error('City-state is already at full charge');
...
export async function upgradeCityState(userId: string, zoneId: string) {
  const zone = await getCityStateById(zoneId);
  if (!zone) throw new Error('City-state not found');
  if (zone.isFallen) throw new Error('Cannot upgrade a fallen city-state');
  if (zone.upgradeLevel >= ZONE_MAX_LEVEL) throw new Error('City-state is already at max level');

  // Require 100% charge to upgrade
  if (zone.charge < zone.maxCharge) throw new Error('City-state must be at full charge to upgrade');
  ...
  // Quest progress
  import('./quests.js').then((m) => m.incrementQuestProgress(userId, 'upgrade', 1)).catch(() => {});

  return { newLevel, newRadius, crystalsSpent: crystalsCost, newBalance };
}

export async function suggestCityState(
  userId: string,
  name: string,
  latitude: number,
  longitude: number,
...
export async function getCityStatePresence(zoneId: string) {
