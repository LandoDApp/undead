...
import {
  RESOURCE_SPAWN_HERB,
  RESOURCE_SPAWN_CRYSTAL,
  RESOURCE_RELIC_CHANCE,
  RESOURCE_LIFETIME_HERB,
  RESOURCE_LIFETIME_CRYSTAL,
  RESOURCE_LIFETIME_RELIC,
  RESOURCE_VALUE_HERB,
  RESOURCE_VALUE_CRYSTAL,
  RESOURCE_VALUE_RELIC,
  RESOURCE_COLLECT_RADIUS,
} from '@undead/shared';
...
const SPAWN_CONFIG: { type: ResourceType; count: number; lifetime: number; value: number }[] = [
...
export async function getOrCreateResourcesInArea(
  center: Coordinate,
  radiusM: number
): Promise<Resource[]> {
  ...
  const cellsNeedingResources = new Set<string>();
  ...
  if (cellsNeedingResources.size > 0) {
    ...
    const ghoulRows = await db
      ...
      .where(
        and(
          inArray(ghouls.gridCell, cellsArr),
          eq(ghouls.isAlive, true)
        )
      );

    // Group ghouls by cell
    const ghoulsByCell = new Map<string, { latitude: number; longitude: number }[]>();
    ...
    for (const cell of cellsArr) {
      ...
      if (!cellGhouls || cellGhouls.length === 0) continue;
      ...
      if (existingRelics === 0 && Math.random() < RESOURCE_RELIC_CHANCE) {
        ...
        spawnPromises.push(
          locateOnNetwork(point).then((snapped) => ({
            ...
            longitude: (snapped ?? point).longitude,
            gridCell: cell,
            resourceType: 'relic' as ResourceType,
...
export async function collectResource(
  ...
  playerLat: number,
  playerLon: number
): Promise<{ collected: boolean; type: ResourceType; amount: number; newBalance: ResourceBalance }> {
  const emptyBalance: ResourceBalance = { herbs: 0, crystals: 0, relics: 0, lifetimeHerbs: 0, lifetimeCrystals: 0, lifetimeRelics: 0 };
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(collectiblePoints.id, resourceId),
        isNull(collectiblePoints.collectedBy)
      )
    );

  const resource = rows[0];
  if (!resource) return { collected: false, type: 'herb', amount: 0, newBalance: emptyBalance };

  // Check expired
  if (resource.expiresAt < Date.now()) return { collected: false, type: 'herb', amount: 0, newBalance: emptyBalance };
  ...
  if (dist > RESOURCE_COLLECT_RADIUS) return { collected: false, type: 'herb', amount: 0, newBalance: emptyBalance };
...
export async function spendHerbs(userId: string, amount: number): Promise<ResourceBalance | null> {
  const balance = await getPlayerResourceBalance(userId);
  if (balance.herbs < amount) return null;
...
export async function spendCrystals(userId: string, amount: number): Promise<ResourceBalance | null> {
  const balance = await getPlayerResourceBalance(userId);
  if (balance.crystals < amount) return null;
...
export async function getPlayerResourceBalance(userId: string): Promise<ResourceBalance> {
  const rows = await db
    .select()
    .from(playerPoints)
    .where(eq(playerPoints.userId, userId));
